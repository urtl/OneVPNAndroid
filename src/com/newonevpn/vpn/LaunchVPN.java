...
public class LaunchVPN extends ListActivity implements OnItemClickListener {
	...
	protected void onStart() {
		super.onStart();
		// Resolve the intent
	//	Toast.makeText(this, "Launch VPN", Toast.LENGTH_SHORT).show();
	...
	private void askForPW(final int type) {
        ...
        if (type == R.string.password) {
        	...
        	//mSelectedProfile.mUsername = "androidtest";//NetSee.mUser;// ((EditText) userpwlayout.findViewById(R.id.username)).getText().toString();

        //	String pw = "t3st4nd";//NetSee.mPass;// ((EditText) userpwlayout.findViewById(R.id.password)).getText().toString();
		...
		if(requestCode==START_VPN_PROFILE) {
			if(resultCode == Activity.RESULT_OK) {
				...
				if(needpw !=0) {
					...
					boolean showlogwindow = Global.sp.getBoolean("chkLogWindow", false);
